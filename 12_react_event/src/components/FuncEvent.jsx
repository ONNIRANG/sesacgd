...
export default function FuncEvent() {
  ...
  const [menu, setMenu] = useState("오늘 점심 뭐 먹지?");
  ...
  const handleEnter = (e) => {
    //e는 리액트 합성 이벤트 개게를 의미
    //target은 해당 이벤트가 발생한 요소에 접근할 수 있음
    //key는 해당 이벤트가 발생하기 위해 눌려진 키보드 키를 의미
    //일반 문자는 process로 enter 는 enter로 값을 가짐
    //code는 해당 이벤트가 발생하기 위해 눌려진 키보드 키를 더 자세히 확인
    // 일반 문자는 'keyA'와 같이 KEY 뒤에 눌려진 키를 확인할 수 있음

    console.log(e);
    if (e.key === "Enter") {
      alert(`점심메뉴는 ${menu}로 결정!`);
    }
  };
  return (
    <>
      ...
      <br />
      {/*input태그에서 변경사항 발생시 해당 input의 value 를 콘솔에 출력 */}
      <input
        type="text"
        value={menu}
        placeholder="오늘 점심 뭐 먹지?"
        onChange={(e) => {
          //여기기서 사용되는 e는 브라우저의 native 이벤트 객체가 아닌 react의 synthetic 이벤트 객체
